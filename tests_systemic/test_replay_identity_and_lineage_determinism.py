...
Tests replay property across system:
- Load fixtures with multiple scenarios
- Process raw events → canonical events
- Make identity decisions
- Establish lineage links
- Replay with same inputs → verify identical results
"""
...
class CanonicalEventProcessor:
    ...
    def __init__(self, normalizer_version="1.0.0", adapter_version="1.0.0", schema_version="1.0.0"):
    ...
    def canonicalize_raw_event(self, raw_event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        canonical = {
            ...
            "external_reference": raw_event.get("external_ref"),
    ...
    def process_event(self, canonical: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Include canonicalization context for determinism per version
        key_event = canonical.copy()
        key_event["_canonicalization_context"] = f"{self.normalizer_version}|{self.adapter_version}|{self.schema_version}"
...
class TestReplayIdentityAndLineageDeterminism:
    ...
    def test_replay_multiple_sources_deterministic(self):
        """
        Systemic: Multiple sources in order → deterministic identity decisions.
        """
    ...
    def test_replay_out_of_order_deterministic(self):
        """
        Systemic: Out-of-order events → deterministic regardless of ingestion order.
        """
        scenario = self._load_scenario("scenario_out_of_order.json")
        
        # First run: process in given order
        processor1 = CanonicalEventProcessor()
        results1 = processor1.replay(scenario["raw_events"])
        
        # Second run: same order (replay)
        processor2 = CanonicalEventProcessor()
        results2 = processor2.replay(scenario["raw_events"])
        ...
        # All events should be ACCEPT (different source_event_ids)
    ...
    def test_replay_duplicate_retries_idempotent(self):
        """
        Systemic: Duplicate retries → 1 ACCEPT + N-1 REJECT_DUPLICATE.
        """
        ...
        # Replay should produce identical decisions
        assert results1["accept_count"] == results2["accept_count"]
        assert results1["reject_duplicate_count"] == results2["reject_duplicate_count"]
        
        # Decision sequence should be identical
        decisions1 = [d["decision"] for d in results1["decisions"]]
        decisions2 = [d["decision"] for d in results2["decisions"]]
        assert decisions1 == decisions2
    ...
    def test_replay_ambiguous_collision_flagged(self):
        """
        Systemic: Ambiguous collision → FLAG_AMBIGUOUS (conservative).
        """
    ...
    def test_replay_lineage_append_only(self):
        """
        Systemic: Lineage links are append-only and deterministic.
        """
        ...
        assert links1 == links2  # Deterministic
        
        # Lineage link types should be identical
    ...
    def test_replay_with_different_versions_produces_different_keys(self):
        """
        Systemic: Different versions → different idempotency keys (deterministic per version).
        """
        scenario = self._load_scenario("scenario_multiple_sources.json")
        ...
        # Different versions → different idempotency keys
        assert keys_v1 != keys_v2, "Different normalizer versions must produce different idempotency keys"
        
        # But: deterministic per version context (replay v2 again)
    ...
    def test_full_replay_end_to_end(self):
        ...
        # All events should pass schema validation
        validator = SchemaValidator()
        for event in results["canonical_events"]:
            schema_result = validator.validate(event)
            assert schema_result.valid, f"Schema validation failed: {schema_result.errors}"
        
        # All ACCEPT events should have unique idempotency keys
        accepted_keys = [
            e["idempotency_key"]
        ...
        # All decisions should have evidence
    ...
    def test_replay_missing_amount_rejected_explicitly(self):
        """
        Systemic: Missing required field (amount) → explicit rejection.
        
        No silent defaults - schema validation must reject.
        """
